/*
rules = [OfflerGoodCodeSyntax, OfflerGoodCodeSemantic]
 */
package example

import scala.util.control.NonFatal

import example.Forbidden._// assert: OfflerGoodCodeSyntax.forbiddenImport

object Foo {
  // --- Rule#3 ---

  import scala.util.control.Breaks// assert: OfflerGoodCodeSyntax.duplicateImporterRef
  ...
  def test1(foo: Foo): Unit = {
    ...
    println {
      if (foo.name.isEmpty) {
        "<none>"
      } else foo.name.drop(1) // must be kept
    }
...
#multiline_block_element2
  ...
  def missingDeclType1() = {// assert: OfflerGoodCodeSemantic.missingDeclType
  ...
  def missingDeclType4(): Unit = {
    def localfn[T, R](z: T)(f: T => Unit)(r: R) = {// assert: OfflerGoodCodeSemantic.missingDeclType
      println(s"r = $r")
      f(z)
    }
  ...
  val preferNoType1: String = "for literal"

  def preferNoType2: String = s"interpol: $preferNoType1"

  // --- Spark rule#3 ---

  val test11: Unit = forbidden1()// assert: OfflerGoodCodeSemantic.forbiddenApply

  val test12: () => Unit = forbidden1 _// assert: OfflerGoodCodeSemantic.forbiddenApply

  val test13: Seq[String] = Seq("foo", "bar").map(Forbidden.forbidden2)// assert: OfflerGoodCodeSemantic.forbiddenApply
}

/* --- Rule#9 --- */
case class Foo(
    name: String,
    age: Int = 1 // assert: OfflerGoodCodeSyntax.caseClassWithDefault
  )
